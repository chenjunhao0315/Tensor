...
int main(int argc, const char * argv[]) {
    ...
    if (argc < 2) {
        printf("Usage ./YoloFastest image_path\n");
        return -1;
    }
...
//    net.summary();
    ...
    const int target_size = (argc > 2) ? std::atoi(argv[2]) : 320;

    auto resize = otter::yolo_pre_process(img, target_size);
    printf("Resize input (%d, %d) to (%d, %d)\n", width, height, (int)resize.size(3), (int)resize.size(2));

    auto ex = net.create_extractor();

    otter::Clock c;
    ex.input("data", resize);

    otter::Tensor pred;
    ex.extract("yolo", pred, 0);

    auto pred_fix = otter::yolo_post_process(pred, width, height);
    ...
    otter::cv::save_image(image, "yolo-fastest");
