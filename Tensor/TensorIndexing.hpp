...
#ifndef TensorIndexing_hpp
#define TensorIndexing_hpp
...
constexpr otter::nullopt_t None = otter::nullopt;
...
struct Slice final {
 public:
  ...
  Slice(
      otter::optional<int64_t> start_index = otter::nullopt,
      otter::optional<int64_t> stop_index = otter::nullopt,
      otter::optional<int64_t> step_index = otter::nullopt) {
    if (!step_index.has_value()) {
      step_ = 1;
    } else {
      step_ = step_index.value();
      OTTER_CHECK(step_ != 0, "slice step cannot be zero");
...
// `otter::indexing::TensorIndex` is used for converting C++ tensor indices such as
...
// Python                  | C++
// -----------------------------------------------------
// `None`                  | `otter::indexing::None`
// `Ellipsis`              | `otter::indexing::Ellipsis`
// `...`                   | `"..."`
// `123`                   | `123`
...
struct TensorIndex final {
  // Case 1: `otter::indexing::None`
  // NOLINTNEXTLINE(clang-analyzer-optin.cplusplus.UninitializedObject)
  TensorIndex(otter::nullopt_t) : type_(TensorIndexType::None) {}
  // Case 2: "..." / `otter::indexing::Ellipsis`
  // NOLINTNEXTLINE(clang-analyzer-optin.cplusplus.UninitializedObject)
  TensorIndex(otter::indexing::EllipsisIndexType)
      : type_(TensorIndexType::Ellipsis) {}
  TensorIndex(const char* str) : TensorIndex(otter::indexing::Ellipsis) {
    // NOLINTNEXTLINE(clang-analyzer-optin.cplusplus.UninitializedObject)
    OTTER_CHECK(
        strcmp(str, "...") == 0,
  ...
  // Case 5: Slice represented in `otter::indexing::Slice` form
  // NOLINTNEXTLINE(clang-analyzer-optin.cplusplus.UninitializedObject)
...
static inline Tensor applySlice(
    ...
    const otter::optional<IntArrayRef>& self_sizes) {
  // TODO: implement negative step
  OTTER_CHECK(step > 0, "step must be greater than zero");
  // See NOTE [nested tensor size for indexing]
  if (self_sizes.has_value()) {
    // Skip this optimization if we are tracing, as the trace may be polymorphic
    // over the shape of the `self` tensor, and we still want to record
    // the slice.
    int64_t length = (self_device == Device::CPU)
        ? (*self_sizes)[dim]
        : self.size(dim);
...
static inline Tensor boolToIndexingTensorCPUOrCUDA(
  ...
  // false as empty.
  if (value) {
    return otter::empty({1}, self.options().dtype(otter::ScalarType::Long)).fill_(0.);
  } else {
    return otter::empty({0}, self.options().dtype(otter::ScalarType::Long));
  }
}
static inline Tensor boolToIndexingTensorNonNativeDeviceType(
  ...
  // false as empty.
  if (value) {
    return otter::zeros({1}, self.options().dtype(otter::ScalarType::Long));
  } else {
    return otter::empty({0}, self.options().dtype(otter::ScalarType::Long));
  }
}
...
static inline Tensor scalarToTensorNonNativeDeviceType(
  ...
  return otter::scalar_tensor(v, options);
}
...
static inline std::vector<otter::optional<Tensor>> typeConvertIndices(
    const Tensor& /*self*/,
    std::vector<Tensor>&& indices) {
  std::vector<otter::optional<Tensor>> converted_inds;
...
static inline int64_t count_specified_dimensions(
  ...
  for (auto& obj : indices) {
    if (obj.is_tensor()) {
      ...
      if (tensor.scalar_type() == otter::ScalarType::Byte || tensor.scalar_type() == otter::ScalarType::Bool) {
        count += tensor.dim();
      } else {
        count++;
      }
...
// - `void set_item(...)`
//
// The rest of the functions are in `otter::indexing::impl` namespace, signifying
...
static inline void copy_to(const Tensor& dst, const Tensor& src) {
  ...
  otter::MaybeOwned<Tensor> b_src = expand_inplace(dst, src_view, "setitem");
  dst.copy_(*b_src);
}
...
static inline Tensor handleDimInMultiDimIndexing(
    ...
    std::vector<Tensor>& outIndices,
    bool disable_slice_optimization,
    const Device& original_tensor_device,
    const otter::optional<IntArrayRef>& prev_dim_result_sizes) {
  if (index.is_integer()) {
    ...
    if (tensor.dim() == 0 &&
        otter::isIntegralType(scalar_type, /*includeBool=*/true)) {
      if (scalar_type != otter::ScalarType::Byte && scalar_type != otter::ScalarType::Bool) {
        ...
        if (scalar_type == otter::ScalarType::Bool) {
          impl::recordTensorIndex(
              impl::boolToIndexingTensor(
    ...
    OTTER_INTERNAL_ASSERT(false, "Invalid TensorIndex type");
  }
}
namespace impl {
...
static inline Tensor applySlicing(
  ...
  Tensor result = self;
  for (const auto i : otter::irange(indices.size())) {
    auto& obj = indices[i];
    // See NOTE [nested tensor size for indexing]
    otter::optional<IntArrayRef> result_sizes = otter::optional<IntArrayRef>(result.sizes());
...
static inline Tensor get_item(
  ...
  // tensor size
  otter::optional<IntArrayRef> self_sizes = otter::optional<IntArrayRef>(self.sizes());
  ...
  if (indices.size() == 1) {
    ...
    if (index.is_integer()) {
      ...
      return otter::native::alias(self);
    } else if (index.is_boolean()) {
