...
static inline void vectorized_reduction(char** data, int64_t n, int64_t stride,
  ...
  for (const auto i : otter::irange(1, n)) {
  ...
  if (reduce) {
    ...
    acc[0].store(buffer);
    for (const auto j : otter::irange(1, Vec::size())) {
...
static void set_results(const std::tuple<res_t...>& result, const TensorIterator &iter, const int num_outputs) {
  ...
  assert((size_t)num_outputs == result_size);
}
template <typename T, typename... Args>
struct all_same : otter::conjunction<
  std::is_same<T, Args>...
> {};
...
void binary_kernel_reduce(TensorIterator& iter, ops_t ops, init_t init) {
  ...
  iter.foreach_reduced_elt([&ops, &init, num_outputs](TensorIterator &sub_iter) {
    ...
    if (numel < otter::GRAIN_SIZE || otter::get_num_threads() == 1 ||
      ...
      int max_threads = otter::get_num_threads();
      ...
      otter::parallel_for(0, numel, otter::GRAIN_SIZE,
        [&](int64_t begin, int64_t end) {
          auto& acc = buffer[otter::get_thread_num()];
          acc = reduction_body(acc, begin, end);
        }
      );
      for (const auto i : otter::irange(max_threads)) {
...
// can be used.
static inline bool is_reduce_lastdim(TensorIterator& iter) {
...
void binary_kernel_reduce_lastdim(TensorIterator& iter, reduce_func_t reduce_op) {
  auto shape = iter.shape();
  int64_t dim_size = shape[0];
  int64_t grain_size = std::max((int64_t) 1, otter::GRAIN_SIZE / dim_size);
