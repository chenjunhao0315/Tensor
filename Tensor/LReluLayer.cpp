...
int LReluLayer::forward_inplace(Tensor &bottom_blob, const NetOption &opt) const {
    if (opt.use_non_lib_optimize && bottom_blob.scalar_type() == otter::ScalarType::Float) {
        auto input_a = bottom_blob.accessor<float, 4>()[0];
            
        int64_t size = bottom_blob.size(2) * bottom_blob.size(3);
            
        otter::parallel_for(0, bottom_blob.size(1), 0, [&](int64_t begin, int64_t end) {
            for (const auto q : otter::irange(begin, end)) {
                float* ptr = input_a[q].data();
