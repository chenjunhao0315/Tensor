...
mask_scatter_add(const scalar_t *src, scalar_t* base_addr,
  ...
  # pragma unroll
  #endif
  for (const auto i : otter::irange(len)) {
    if (mask[i] & 0x01) {
...
struct ApplyGridSample<scalar_t, 2, GridSamplerInterpolation::Bilinear,
  ...
  inline void forward(TensorAccessor<scalar_t, 3>& out_slice,
    ...
    # pragma unroll
    #endif
    for (const auto c : otter::irange(C)) {
  ...
  inline void backward(TensorAccessor<scalar_t, 3>* gInp_slice_ptr,
    ...
    for (const auto c : otter::irange(C)) {
      ...
      if (input_requires_grad) {
        OTTER_INTERNAL_ASSERT(gInp_slice_ptr);
...
struct ApplyGridSample<scalar_t, 2, GridSamplerInterpolation::Bicubic,
  ...
  inline void backward(TensorAccessor<scalar_t, 3>* gInp_slice_ptr,
    ...
    for (const auto c : otter::irange(C)) {
      ...
      auto gOut = Vec::loadu(gOut_slice[c].data() + offset, len);
      for (const auto i : otter::irange(4)) {
        for (const auto j : otter::irange(4)) {
          auto xx = ix + Vec(-1 + i);
          auto yy = iy + Vec(-1 + j);
          if (input_requires_grad) {
            auto gInp_slice_C_ptr = (*gInp_slice_ptr)[c].data();
...
static inline void grid_sample_2d_grid_slice_iterator(
  ...
  if (otter::geometry_is_contiguous({out_H, out_W, 2}, {grid_sH, grid_sW, grid_sCoor})) {
    // Case 1:
    // Grid is contiguous.
    // Strategy: Sequentially load two vectors at the same time, and get,
    //           e.g.,  {x0, y0, x1, y1}, {x2, y2, x3, y3}. Then we use
    //           otter::vec::deinterleave2 to get x and y vectors.
    ...
    if (otter::geometry_is_contiguous({out_H, out_W}, {grid_sH, grid_sW})) {
    ...
    //           otter::vec::gather to load the x and y vectors.
...
void grid_sampler_2d_cpu_kernel_impl(
  ...
  auto grain_size = spatial_size == 0 ? (N + 1)
                                      : otter::divup(otter::GRAIN_SIZE, spatial_size * 4 /* 2d * 2 tensors*/);
...
#define HANDLE_INTERP(interp, align_corners)                                   \
  ...
  OTTER_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grid_sampler_2d_cpu_kernel_impl", [&] {
...
void grid_sampler_2d_backward_cpu_kernel_impl(
  ...
  auto grain_size = spatial_size == 0 ? (N + 1)
                                      : otter::divup(otter::GRAIN_SIZE, spatial_size * 10 /* 2d * 5 tensors*/);
...
#define HANDLE_INTERP(interp, align_corners, input_requires_grad)           \
  ...
  OTTER_DISPATCH_FLOATING_TYPES(input.scalar_type(), "grid_sampler_2d_backward_cpu_kernel_impl", [&] {
