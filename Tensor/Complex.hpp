...
// otter::complex internally. The end state is that all operators will use
// otter::complex internally.  Until then, there may be some hacks to support all
// variants.
//
//
...
// namespace `otter::complex_literals`. Our custom literals does not follow the
...
//
//
// TODO(@zasdfgbnm): otter::complex<otter::Half> is not currently supported,
// because:
//  - lots of members and functions of otter::Half are not constexpr
...
#if defined(__CUDACC__) || defined(__HIPCC__)
  template <typename U>
  explicit complex(const thrust::complex<U>& other)
      : real_(other.real()), imag_(other.imag()) {}
// NOTE can not be implemented as follow due to ROCm bug:
//   explicit complex(const thrust::complex<U> &other):
//   complex(other.real(), other.imag()) {}
#endif
  // Use SFINAE to specialize casting constructor for otter::complex<float> and
  // otter::complex<double>
...
// Define operators between integral scalars and otter::complex. std::complex does
...
constexpr otter::complex<fT> operator+(const otter::complex<fT>& a, const iT& b) {
  return a + static_cast<fT>(b);
}
template <typename fT, typename iT, COMPLEX_INTEGER_OP_TEMPLATE_CONDITION>
constexpr otter::complex<fT> operator+(const iT& a, const otter::complex<fT>& b) {
