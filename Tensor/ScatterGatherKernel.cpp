...
#include "ScatterGatherKernel.hpp"
...
struct _cpu_scatter_gather_dim_loop {
  ...
  void operator()(
    ...
    for (const auto i : otter::irange(index_dim_size)) {
      ...
      OTTER_CHECK(idx_dim >= 0 && idx_dim < index_upper_bound,
...
struct cpu_scatter_gather_base_kernel {
  ...
  void operator()(const Tensor& self, int64_t dim,
    ...
    int64_t grain_size = std::max((int64_t) 1, otter::GRAIN_SIZE / index_dim_size);
    OTTER_DISPATCH_ALL_TYPES_AND(
      ...
      "scatter_gather_scalar_cpu", [&] {
        ...
        auto loop = [&](char** data, const int64_t* strides, int64_t n) {
          ...
          if (dim== self.dim() - 1) {
            for (const auto nelem : otter::irange(n)) {
  ...
  void operator()(const Tensor& self, int64_t dim,
    ...
    auto index_upper_bound = is_scatter_like ? self_dim_size : src_dim_size;
    int64_t grain_size = std::max((int64_t) 1, otter::GRAIN_SIZE / index_dim_size);
    OTTER_DISPATCH_ALL_TYPES(iter.dtype(),
