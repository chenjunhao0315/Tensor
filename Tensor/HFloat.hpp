...
struct alignas(2) HFloat {
    ...
    constexpr HFloat(unsigned short bits, from_bits_t) : x(bits){};
    inline HFloat(float value);
...
overflows(From f) {
    ...
    if (!limit::is_signed && std::numeric_limits<From>::is_signed) {
        ...
        (otter::is_negative(f) && -static_cast<uint64_t>(f) > limit::max());
    } else {
        return otter::less_than_lowest<To>(f) || greater_than_max<To>(f);
    }
}
template <typename To, typename From>
