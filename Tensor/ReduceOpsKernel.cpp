...
Vectorized<acc_t> load_reduce_vec(const scalar_t* data, F reduce, acc_t ident) {
  ...
  acc.fill(ident);
  for (const auto k : otter::irange(vstride)) {
    for (const auto i : otter::irange(vacc_t::size())) {
...
struct LoadPolicy {
  ...
  static scalar_t load(const char * OTTER_RESTRICT data, int64_t stride, int64_t index) {
...
struct LoadPolicy<Vectorized<scalar_t>> {
  ...
  static Vectorized<scalar_t> load(const char * OTTER_RESTRICT data, int64_t stride, int64_t index) {
...
struct CastLoadPolicy {
  ...
  static acc_t load(const char * OTTER_RESTRICT data, int64_t stride, int64_t index) {
...
struct InnerSumCastLoadPolicy {
  ...
  static vacc_t load(const char * OTTER_RESTRICT data, int64_t stride, int64_t index) {
...
struct NanSumLoadPolicy {
  ...
  static scalar_t load(const char * OTTER_RESTRICT data, int64_t stride, int64_t index) {
    ...
    return otter::_isnan(val) ? scalar_t(0) : val;
  }
};
template <typename scalar_t>
struct NanSumLoadPolicy<Vectorized<scalar_t>> {
  ...
  static vec_t load(const char * OTTER_RESTRICT data, int64_t stride, int64_t index) {
...
struct NanSumCastLoadPolicy {
  ...
  static acc_t load(const char * OTTER_RESTRICT data, int64_t stride, int64_t index) {
    ...
    return otter::_isnan(val) ? acc_t(0) : val;
  }
};
template <typename vec_t, typename vacc_t>
...
struct CastStoreAccumulate {
  static void store(char * OTTER_RESTRICT data, int64_t stride, int64_t index, acc_t value) {
    auto * ptr = reinterpret_cast<scalar_t*>(data + index * stride);
    *ptr += value;
  }
};
template <typename StorePolicy, typename scalar_t>
static void store(char * OTTER_RESTRICT data, int64_t stride, int64_t index, scalar_t value) {
  StorePolicy::store(data, stride, index, value);
}
template <typename StorePolicy, typename scalar_t, size_t numel>
static void store(char * OTTER_RESTRICT data, int64_t stride, int64_t index,
                  const std::array<scalar_t, numel> &values) {
  auto *base_ptr = data + stride * index;
  for (const auto k : otter::irange(numel)) {
    auto val = values[k];
...
std::array<scalar_t, nrows> multi_row_sum(
  ...
  for (; i + level_step <= size;) {
    for (int64_t j = 0; j < level_step; ++j, ++i) {
      ...
      # pragma unroll
      #endif
      for (const auto k : otter::irange(nrows)) {
        acc[0][k] += LoadPolicy::load(sum_base, col_stride, k);
      }
    }
    for (const auto j : otter::irange(1, num_levels)) {
      #if !defined(COMPILING_FOR_MIN_SIZE)
      # pragma unroll
      #endif
      for (const auto k : otter::irange(nrows)) {
...
scalar_t row_sum(const char * OTTER_RESTRICT in_data,
...
void vectorized_inner_sum(
    // NOLINTNEXTLINE(modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays)
    char * OTTER_RESTRICT data[2], int64_t outer_stride, int64_t out_stride,
  ...
  for (const auto j : otter::irange(size1)) {
    ...
    for (const auto k : otter::irange(partials.size())) {
...
void scalar_inner_sum(
    // NOLINTNEXTLINE(modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays)
    char * OTTER_RESTRICT data[2], int64_t in_strides[2], int64_t out_stride,
    int64_t size0, int64_t size1) {
  for (const auto j : otter::irange(size1)) {
...
void vectorized_outer_sum(
    // NOLINTNEXTLINE(modernize-avoid-c-arrays,cppcoreguidelines-avoid-c-arrays)
    char * OTTER_RESTRICT data[2], int64_t inner_stride, int64_t out_stride,
    int64_t size0, int64_t size1) {
  using vacc_t = Vectorized<acc_t>;
  constexpr int64_t scalar_stride = ScalarLoadPolicy::memsize();
  constexpr int64_t vec_stride = VecLoadPolicy::memsize();
  ...
  for (; j + nrows * vacc_t::size() <= size1; j += nrows * vacc_t::size()) {
    ...
    for (const auto i : otter::irange(nrows)) {
    ...
    [&](char** data, const int64_t* strides, int64_t size0, int64_t size1) {
      ...
      if (out_strides[0] != 0 && out_strides[1] != 0) {
        ...
        UNARY_OUTER_LOOP(data, outer_strides, size1, [&] {
          ...
          otter::if_constexpr<ignore_nan>(
            [&](auto) {
              basic_loop(ptrs, inner_strides, 0, size0, [](scalar_t a, scalar_t b) {
                auto a_notnan = otter::_isnan(a) ? scalar_t(0) : a;
                auto b_notnan = otter::_isnan(b) ? scalar_t(0) : b;
      ...
      OTTER_INTERNAL_ASSERT(out_strides[0] == 0);
      using vec_t = Vectorized<scalar_t>;
      using acc_t = otter::acc_type<scalar_t, true>;
      using vacc_t = Vectorized<acc_t>;
...
void sum_kernel_impl(TensorIterator &iter) {
  if (isIntegralType(iter.dtype(), /*includeBool=*/ true)) {
    OTTER_DISPATCH_INTEGRAL_TYPES_AND(ScalarType::Bool, iter.dtype(), "sum_cpu",
      [&] {
  ...
  OTTER_DISPATCH_FLOATING_TYPES_AND(
      ScalarType::HFloat, iter.dtype(), "sum_cpu", [&] {
