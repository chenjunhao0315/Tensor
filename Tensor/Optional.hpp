...
#endif
#if OTTER_CLANG_HAS_WARNING("-Wimplicit-int-conversion")
OTTER_CLANG_DIAGNOSTIC_IGNORE("-Wimplicit-int-conversion")
#endif
...
// keep the size of otter::optional::ArrayRef<T> down to 16 bytes, which
...
template <typename T>
struct is_arrayref<otter::ArrayRef<T>> : std::true_type {};
} // namespace detail_
...
namespace std {
template <typename T>
struct hash<otter::optional<T>> {
  typedef typename hash<T>::result_type result_type;
  typedef otter::optional<T> argument_type;
